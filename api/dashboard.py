"""
API Dashboard - Sunter Dashboard Pro (V12.31 Robust Sync)
Update: 2026-01-19
---------------------------------------------------------------------------
Pembaruan Strategis:
1. Robust Realization Query: Menarik data CURRENT & HISTORY dari collection_harian 
   agar box "Lapangan" tidak 0 jika terjadi anomali label kategori.
2. N+1 Shift Alignment: Memastikan Dashboard Januari menarik realisasi Desember.
3. Total Realisasi Fix: Menggabungkan Bank (Undue) & Lapangan (Current) secara akurat.
4. Recovery Analytics: Memisahkan penagihan piutang lama (Recovery) dari target berjalan.
"""
...
def get_pusat_kendali():
    ...
    try:
        ...
        # [3] REALISASI SINERGI (FIX CURRENT 0)
        # Kami menggunakan IN ('CURRENT', 'HISTORY') untuk memastikan data harian 
        # tetap muncul di box Dashboard walaupun label kategorinya berubah.
        query_realisasi = """
            SELECT 
                ...
                (SELECT COALESCE(SUM(nominal), 0) FROM collection_harian 
                 WHERE periode = ? AND kategori = 'HISTORY') as rec_coll
        """
        ...
        total_recovery = res_realisasi['rec_mb'] + res_realisasi['rec_coll']
        
        # Realisasi murni untuk Dashboard (Bank + Lapangan)
