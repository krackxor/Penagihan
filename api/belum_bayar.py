...
def apply_pro_watermark(image_path, data):
    """Watermark Profesional - © Khoirul Anwar"""
    try:
        ...
        try:
            ...
            font_small = ImageFont.truetype("arial.ttf", int(font_size * 0.7))
        except:
        ...
        draw.text((margin, curr_y), f"PETUGAS: {data['petugas']} | {data['nomen']}", fill=(255, 255, 255), font=font)
        ...
        # Copyright
        draw.text((width // 2 - 150, height - 30), "© COPYRIGHT KHOIRUL ANWAR - SUNTER PRO", fill=(200, 200, 200, 150), font=font_small)
...
def register_belum_bayar_routes(app, get_db):
    ...
    def get_list():
        ...
        # QUERY UTAMA: Join master_pelanggan dengan rute_petugas berdasarkan PCEZ
        # Gunakan TRIM dan UPPER untuk memastikan data rute cocok
        query = """
            SELECT 
                m.nomen, m.nama, m.nominal, m.pcez, m.block,
                IFNULL(r.petugas, 'Belum Diatur') as nama_petugas
            FROM master_pelanggan m
            LEFT JOIN rute_petugas r ON TRIM(UPPER(m.pcez)) = TRIM(UPPER(r.pcez))
            LEFT JOIN kunjungan_petugas k ON m.nomen = k.nomen
            WHERE m.tipe = 'MC' 
            AND k.nomen IS NULL
        """
        params = []
        if petugas_filter != 'all' and petugas_filter != '':
            query += " AND r.petugas = ?"
    ...
    def get_tabs():
        db = get_db()
        # Mengambil semua petugas yang terdaftar di rute
    ...
    def simpan():
        ...
        db.execute("""
            INSERT INTO kunjungan_petugas (nomen, petugas_name, keterangan, no_hp, catatan, janji_bayar_dt, foto_path)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (f.get('nomen'), f.get('petugas'), f.get('keterangan'), f.get('no_hp'), f.get('catatan'), f.get('janji_bayar_dt'), filename))
        db.commit()
