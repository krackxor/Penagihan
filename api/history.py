"""
History API Endpoints - Area Service Integrated System (V8.2 Intelligence Edition)
Integritas Data & Audit Geospasial:
1. Smart Periode Parser: Konversi otomatis format YYYY-MM (HTML5) ke MM-YYYY (DB Standard).
2. Nomen Sanitizer: Integrasi clean_nomen untuk mencegah kegagalan join akibat format Excel.
...
def simpan_kunjungan():
    ...
    nomen = clean_nomen(raw_nomen) # Mencegah error .0 dari Excel
    ...
    # Penangkapan Koordinat Geospasial dengan Sanitizer
    ...
    conn = get_db_connection()
    try:
        # --- LANGKAH 1: SNAPSHOT DATA MASTER (Integritas Data Fisik) ---
        # Menggunakan join periode terbaru untuk akurasi saldo
        p_info = conn.execute("""
        ...
        # --- LANGKAH 4: TRANSAKSI DATABASE (WIB Standard) ---
        conn.execute("""
...
def list_kunjungan():
    ...
    # --- SINKRONISASI PERIODE (HTML5 TO DB) ---
    ...
    conn = get_db_connection()
    try:
        # Optimized query menggunakan INDEX periode
        query = """
        ...
        # Filter Role: Petugas hanya melihat miliknya, Admin/Guest melihat Global
        if role == 'petugas':
        ...
        for row in rows:
            d = dict(row)
            # Normalisasi Geospasial untuk Frontend
